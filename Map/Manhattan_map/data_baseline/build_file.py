...
def init_routes(density):
    init_flow = '  <flow id="i_%s" departPos="random_free" from="%s" to="%s" begin="0" end="1" departLane="%d" departSpeed="random" arrivalPos ="10" number="%d" type="type1"/>\n'
    output = ''
...
def output_config(thread=None):
    ...
    str_config += '    <additional-files value="exp.add1.xml"/>\n'
    str_config += '  </input>\n  <time>\n'
...
def main():
    ...
    write_file('exp.add1.xml', output_ild(ild))
