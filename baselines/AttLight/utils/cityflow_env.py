...
class CityFlowEnv:
    ...
    def reset(self,seedNum = 0,server = 0):
        ...
        cityflow_config = {
            ...
            "rlTrafficLight": True,
            "saveReplay": False,
            "roadnetLogFile": "frontend/web/roadnetLogFile{}.json".format(self.server_number),
            "replayLogFile": "frontend/web/replayLogFile{}.txt".format(self.server_number)
        }
        # print(cityflow_config)
        with open(os.path.join(self.path_to_work_directory, "cityflow{}.config".format(server)), "w") as json_file:
            json.dump(cityflow_config, json_file)

        self.eng = engine.Engine(os.path.join(self.path_to_work_directory, "cityflow{}.config".format(server)), thread_num=1)
    ...
    def _adjacency_extraction(self):
        ...
        with open("{0}".format(file)) as json_data:
            ...
            for i,id in enumerate(traffic_light_node_dict.keys()):
                traffic_light_node_dict[id]["total_inter_num"] = inter_id_to_index
                traffic_light_node_dict[id]["neighbor_ENWS"] = []
                for j in range(4):
                    road_id = id.replace("intersection", "road")+"_"+str(j)
                    if edge_id_dict[road_id]["to"] not in traffic_light_node_dict.keys():
                        traffic_light_node_dict[id]["neighbor_ENWS"].append(None)
                    else:
                        traffic_light_node_dict[id]["neighbor_ENWS"].append(edge_id_dict[road_id]["to"])

                self.id_to_index[id] = i
                self.index_to_id[i] = id
                self.action_space[id] = self.dic_traffic_env_conf['NUM_PHASES']
